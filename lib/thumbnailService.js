...
async function validateAndResizePng(buffer, maxWidth, maxHeight) {
	const fallbackDimensions = parsePngDimensions(buffer);
	let image;
	try {
		...
		if (fallbackDimensions) {
			return {
				buffer: buffer,
				width: Math.min(fallbackDimensions.width, maxWidth || fallbackDimensions.width),
				height: Math.min(fallbackDimensions.height, maxHeight || fallbackDimensions.height)
			};
		}
	...
	try {
		...
		throw createHttpError(422, error && error.message ? error.message : 'Converted payload is not a valid image.');
	}
}
...
async function renderOfficeThumbnail(options) {
	...
	const storageId = options.storageId || (options.storageContext === 'shared' ? 'documents' : options.storageContext) || 'documents';
	...
	const cached = await getCachedThumbnail(options.documentRoot, fileId, version, storageId);
	if (cached) {
	...
	const accessToken = createAccessToken({
		...
		claims: {
			...
			canWrite: false,
			canRename: false,
			storageId: options.storageId || 'documents'
		}
	});

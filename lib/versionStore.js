...
// Keeps the version history bounded by age and count while always preserving the newest
// version (index 0, since new versions are unshifted) and any version the user has labeled.
async function pruneVersions(documentRoot, index) {
	if (index.versions.length <= 1) {
		return;
	}

	const maxAgeMs = config.versionMaxAgeDays * 24 * 60 * 60 * 1000;
	...
	index.versions.forEach((version, position) => {
		if (version === newest || version.label) {
			kept.push(version);
			return;
		}

		const createdAt = Date.parse(version.createdAt);
		const tooOld = Number.isFinite(createdAt) && createdAt < cutoff;
		const overCount = position >= config.versionMaxCount;
		if (tooOld || overCount) {
...
async function createVersionSnapshot(documentRoot, document, actor) {
	...
	await pruneVersions(documentRoot, index);
...
async function deleteAllVersions(documentRoot, fileId) {
	const index = await loadIndex(documentRoot, fileId);
	...
	try {
		await fs.unlink(getIndexFile(documentRoot, fileId));
	} catch (error) {
		if (error.code !== 'ENOENT') {
			throw error;
		}

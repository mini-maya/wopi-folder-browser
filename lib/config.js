...
function parseSharedStorageMode(value) {
	const normalized = String(value || '').trim().toLowerCase();
	if (!normalized) {
		return 'disabled';
	}
	if (['disabled', 'auth', 'readonly', 'readwrite'].includes(normalized)) {
		return normalized;
	}
	throw new Error('SHARED_STORAGE_MODE must be one of: disabled, auth, readonly, readwrite.');
}

// Local development convenience fallback only: production deployments should set their
// storage roots explicitly via environment variables.
const defaultLocalDevDataRoot = path.join(__dirname, '..', 'wopi_dev_folder');

module.exports = {
	documentRoot: path.resolve(process.env.DOCUMENT_ROOT || path.join(defaultLocalDevDataRoot, 'document-storage')),
	sharedStorageMode: parseSharedStorageMode(process.env.SHARED_STORAGE_MODE),
	sharedStorageRoot: path.resolve(process.env.SHARED_STORAGE_ROOT || path.join(process.env.DOCUMENT_ROOT || path.join(defaultLocalDevDataRoot, 'document-storage'), 'shared-storage')),
	templateRoot: path.resolve(process.env.TEMPLATE_ROOT || path.join(process.env.DOCUMENT_ROOT || path.join(defaultLocalDevDataRoot, 'document-storage'), '.templates')),
	...
	externalStorageRoot: path.resolve(process.env.EXTERNAL_STORAGE_ROOT || '/external/storage'),
	externalStorageName: process.env.EXTERNAL_STORAGE_NAME || 'External Storage',
	externalStorageEnabled: parseBooleanFlag(process.env.EXTERNAL_STORAGE_ENABLED, true),
	externalStorageReadOnly: parseBooleanFlag(process.env.EXTERNAL_STORAGE_READ_ONLY, false),

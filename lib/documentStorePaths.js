...
function getStorageNamespace(documentRoot) {
  return crypto.createHash('sha256').update(path.resolve(documentRoot)).digest('hex').slice(0, 12);
}
...
function getRegistryEntryPath(entryValue) {
  if (typeof entryValue === 'string') {
    return normalizeRelativePath(entryValue);
  }
  if (entryValue && typeof entryValue === 'object' && typeof entryValue.path === 'string') {
    return normalizeRelativePath(entryValue.path);
  }
  return null;
}

function normalizeRegistryEntry(entryValue, fallbackPath = null) {
  const normalizedPath = getRegistryEntryPath(entryValue) || fallbackPath;
  if (!normalizedPath) {
    return null;
  }
...
async function ensurePathId(documentRoot, relativePath) {
  ...
  const existingEntry = Object.entries(registry.entries).find((entry) => {
    const entryPath = getRegistryEntryPath(entry[1]);
  ...
  const generatedId = `${getStorageNamespace(documentRoot)}-${crypto.randomUUID()}`;
...
async function resolveRelativePathFromId(documentRoot, fileId) {
  const registry = await loadRegistry(documentRoot);
  const entryValue = registry.entries[fileId];
  const resolvedPath = getRegistryEntryPath(entryValue);
  if (resolvedPath) {
    return resolvedPath;
  }

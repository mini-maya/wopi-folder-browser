...
function normalizeMountIds(mountIds) {
	return Array.isArray(mountIds)
		? mountIds
			.filter((mountId) => mountId !== null && mountId !== undefined)
			.map((mountId) => String(mountId).trim())
			.filter((mountId) => mountId)
		: [];
}
...
function toPublicUser(user) {
	return {
		...
		mounts: Array.isArray(user.mounts) ? user.mounts.slice() : [],
...
async function createUser(documentRoot, options) {
	...
	const user = {
		...
		mounts: normalizeMountIds(options.mounts),
		created_at: now,
		updated_at: now
	};
...
async function setUserMounts(documentRoot, userId, mountIds) {
	const normalizedMounts = normalizeMountIds(mountIds);
	return updateUser(documentRoot, userId, (entry) => ({
		...entry,
...
async function getUserMounts(documentRoot, userId) {
	const user = await getUserById(documentRoot, userId);
	if (!user) {
		return [];
	}
	return Array.isArray(user.mounts) ? user.mounts : [];
}

...
const { addRecycledEntry } = require('./recycleStore');
const {
...
async function recycleDocument(documentRoot, document, options = {}) {
	if (document.isDirectory) {
		return null;
	}

	const snapshot = await createVersionSnapshot(documentRoot, document, {
		id: options.actor?.id || 'system',
		name: options.actor?.name || 'System'
	});
	return addRecycledEntry(documentRoot, {
		fileId: document.id,
		...
		versionPath: snapshot.storageFileName,
		snapshotId: snapshot.id,
		versionSize: snapshot.size,
		mimeType: document.mimeType || null,
		previewVersion: document.version
	});
}
...
async function recycleDirectoryTree(documentRoot, relativePath, options = {}) {
	const targetDocument = await getTargetDocument(documentRoot, relativePath);
	if (!targetDocument || !targetDocument.isDirectory) {
		return [];
	}

	const documentsToDelete = (await listDocuments(documentRoot))
		.filter((document) => document.relativePath === relativePath || isPathWithinPrefix(document.relativePath, relativePath))
		.sort((left, right) => right.relativePath.length - left.relativePath.length);

	for (const document of documentsToDelete) {
		try {
			await deleteDocumentWithOptions(documentRoot, document.id, options);
		} catch (error) {
...
async function integrateDirectoryTree(documentRoot, sourceRelativePath, targetRelativePath, options = {}) {
	...
	for (const entry of entries) {
		...
		if (entry.isDirectory()) {
			...
			if (!existingTarget) {
				...
				if (options.operation === 'move') {
					await remapRegistryPaths(documentRoot, sourceEntryRelativePath, targetEntryRelativePath);
				}
				continue;
			}
			if (!existingTarget.isDirectory) {
				const resolution = (options.fileConflictResolutions && options.fileConflictResolutions[targetEntryRelativePath]) || options.fileConflictResolution;
				if (!resolution) {
					const sourceDoc = { name: entry.name, isDirectory: true, size: 0, updatedAt: new Date().toISOString(), kind: 'folder' };
					throw createFileConflictError(options.operation || 'copy', sourceDoc, existingTarget);
				}
				if (resolution === 'skip') {
					continue;
				}
				...
				if (resolution === 'keep_both') {
					...
					if (options.operation === 'move') {
						await remapRegistryPaths(documentRoot, sourceEntryRelativePath, nextTargetRelativePath);
					}
					continue;
				}
				continue;
			}
			const directoryResolution = (options.directoryConflictResolutions && options.directoryConflictResolutions[targetEntryRelativePath]) || options.directoryConflictResolution;
			if (!directoryResolution) {
				const sourceDoc = { name: entry.name, isDirectory: true, size: 0, updatedAt: new Date().toISOString(), kind: 'folder' };
				throw createDirectoryConflictError(options.operation || 'copy', sourceDoc, existingTarget);
			}
			if (directoryResolution === 'skip') {
				continue;
			}
			if (directoryResolution === 'replace') {
				await recycleDirectoryTree(documentRoot, targetEntryRelativePath, options);
				await copyDirectoryRecursive(sourceEntryAbsolutePath, targetEntryAbsolutePath);
				if (options.operation === 'move') {
					await remapRegistryPaths(documentRoot, sourceEntryRelativePath, targetEntryRelativePath);
				}
			...
			if (directoryResolution === 'integrate') {
				await integrateDirectoryTree(documentRoot, sourceEntryRelativePath, targetEntryRelativePath, {
					...
					directoryConflictResolution: options.directoryConflictResolution,
					directoryConflictResolutions: options.directoryConflictResolutions,
					fileConflictResolution: options.fileConflictResolution,
					fileConflictResolutions: options.fileConflictResolutions
				});
				continue;
			}
			continue;
		...
		const resolution = (options.fileConflictResolutions && options.fileConflictResolutions[targetEntryRelativePath]) || options.fileConflictResolution;
		if (!resolution) {
			const sourceDoc = await buildMetadata(documentRoot, await ensurePathId(documentRoot, sourceEntryRelativePath), sourceEntryRelativePath, await fs.stat(sourceEntryAbsolutePath));
			throw createFileConflictError(options.operation || 'copy', sourceDoc, existingTarget);
		}
		if (resolution === 'skip') {
			continue;
		...
		if (resolution === 'overwrite') {
			if (!existingTarget.isDirectory) {
				await deleteDocumentWithOptions(documentRoot, existingTarget.id, options);
			}
			if (entry.isFile()) {
				await fs.copyFile(sourceEntryAbsolutePath, targetEntryAbsolutePath);
			}
			if (options.operation === 'move') {
				...
				await ensurePathId(documentRoot, targetEntryRelativePath);
			}
			continue;
		}
		if (resolution === 'keep_both') {
			...
			if (entry.isFile()) {
				...
				if (options.operation === 'move') {
					...
					await ensurePathId(documentRoot, nextTargetRelativePath);
				}
			}
			continue;
		}
...
async function renameOrMoveDocument(documentRoot, fileId, options = {}) {
	...
	if (existingTarget && existingTarget.id !== document.id) {
		...
		if (resolution === 'replace') {
			if (document.isDirectory && existingTarget.isDirectory) {
				await recycleDirectoryTree(documentRoot, targetRelativePath, options);
				await fs.mkdir(path.dirname(targetPath), { recursive: true });
			...
			await deleteRegistryEntries(documentRoot, targetRelativePath);
...
async function copyDocument(documentRoot, fileId, options = {}) {
	...
	const targetRelativePath = getTargetRelativePath(targetDirectory, targetName);
	if (targetRelativePath === document.relativePath) {
		throw createHttpError(400, 'A file cannot be copied onto itself.');
	}
	if (!document.isDirectory && !getMimeType(targetRelativePath)) {
		throw createHttpError(400, 'The target file type is not supported.');
	}
...
async function deleteDocument(documentRoot, fileId, options = {}) {
	return deleteDocumentWithOptions(documentRoot, fileId, options);
}

async function recycleFilesInDirectory(documentRoot, relativePath, options = {}) {
	const allDocuments = await listDocuments(documentRoot);
	const contained = allDocuments.filter(
		(doc) => !doc.isDirectory && (doc.relativePath === relativePath || isPathWithinPrefix(doc.relativePath, relativePath))
	);
	for (const doc of contained) {
		try {
			await recycleDocument(documentRoot, doc, options);
		} catch (error) {
			if (error && (error.status === 404 || error.code === 'ENOENT')) {
				continue;
			}
...
async function deleteDocumentWithOptions(documentRoot, fileId, options = {}) {
	const document = await getDocumentById(documentRoot, fileId);
	if (document.isDirectory) {
		await recycleFilesInDirectory(documentRoot, document.relativePath, options);
		await fs.rm(document.absolutePath, { recursive: true, force: false });
	} else {
		await recycleDocument(documentRoot, document, options);
...
module.exports = {
	...
	deleteDocumentWithOptions: deleteDocumentWithOptions,

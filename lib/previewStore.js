...
function getThumbnailPublicUrl(fileId, version, storageId) {
	const normalizedStorageId = storageId || 'documents';
	return `/storage/${encodeURIComponent(normalizedStorageId)}/thumbnails/${encodeURIComponent(fileId)}/${encodeURIComponent(version)}`;
}

async function getCachedThumbnail(documentRoot, fileId, version, storageId) {
	...
	return {
		...entry,
		thumbnailUrl: getThumbnailPublicUrl(fileId, entry.version, storageId || entry.storageId || 'documents')
	};
}
...
async function storeThumbnail(documentRoot, options) {
	...
	const storageId = options.storageId || 'documents';
	...
	return {
		...state.entries[cacheKey],
		thumbnailUrl: getThumbnailPublicUrl(options.fileId, options.version, storageId)
	};
}

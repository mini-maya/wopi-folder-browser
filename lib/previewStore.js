...
function getThumbnailPublicUrl(fileId, version, mountId) {
	const normalizedMountId = mountId || 'documents';
	return `/mount/${encodeURIComponent(normalizedMountId)}/thumbnails/${encodeURIComponent(fileId)}/${encodeURIComponent(version)}`;
}

async function getCachedThumbnail(documentRoot, fileId, version, mountId) {
	...
	return {
		...entry,
		thumbnailUrl: getThumbnailPublicUrl(fileId, entry.version, mountId || entry.mountId || 'documents')
	};
}
...
async function storeThumbnail(documentRoot, options) {
	...
	const mountId = options.mountId || 'documents';
	...
	return {
		...state.entries[cacheKey],
		thumbnailUrl: getThumbnailPublicUrl(options.fileId, options.version, mountId)
	};
}

...
async function createDocumentsZip(documents) {
	...
	for (const document of sortedDocuments) {
		if (document.isDirectory) {
			await collectFolderEntries(document.absolutePath, document.relativePath, entries);
			continue;
		}
		await collectFileEntry(document.absolutePath, document.relativePath, entries);
	}

	return {
		buffer: buildZipArchive(entries),
		downloadName: 'selected-items.zip'
	};
}
...
async function collectFileEntry(absolutePath, zipPath, entries) {
...
module.exports = {
	createDocumentsZip: createDocumentsZip,
